...
struct Intersection {
    ...
    Intersection(const aoc::point<int> &point, int timeA, int timeB)
        : point(point), time_a(timeA), time_b(timeB), time(timeA + timeB) {}
};
...
struct Segment {
    ...
    Segment(bool vertical, int position, int start, int anEnd, int startTime, int timeDirection)
        : vertical(vertical), position(position), start(start), end(anEnd), start_time(startTime),
          time_direction(timeDirection) {}

    void add_intersections(std::vector<Intersection> &list, const Segment &other) const {
        if (vertical != other.vertical) {
            if (start <= other.position && other.position <= end && other.start <= position && position <= other.end) {

                aoc::point<int> point =
                    vertical ? aoc::point<int>(position, other.position) : aoc::point<int>(other.position, position);

                list.emplace_back(point, start_time + time_direction * (other.position - start),
            ...
            if (position == other.position) {
                ...
                for (int i = max_start; i <= min_end; i++) {
                    aoc::point<int> point = vertical ? aoc::point<int>(position, i) : aoc::point<int>(i, position);
                    list.emplace_back(point, start_time + time_direction * (i - start),
...
void Day3::run_day() {
    ...
    if (!getline(inputfile, line)) {
        inputfile.close();
        AOC_ABORT("Unable to read line 1");
    }
    WireMess mess1(line);

    if (!getline(inputfile, line)) {
        inputfile.close();
        AOC_ABORT("Unable to read line 2");
    }
    WireMess mess2(line);
    ...
    print_result("Part one", "Minimum distance", minDistance, 1337);
    print_result("Part two", "Shortest time", shortest.time, 65356);
}