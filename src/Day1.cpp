#include "Day1.h"
#include <iostream>
#include <string>
#include <vector>

void Day1::run_day() {
    fs::ifstream inputfile(input / "day1.txt");
    if (!inputfile.is_open()) {
        AOC_ABORT("Unable to open day1.txt");
    }

    std::vector<int> masses;

    std::string line;
    while (getline(inputfile, line)) {
        masses.push_back(std::stoi(line));
    }
    inputfile.close();

    print_trivia("Module count", (int)masses.size());

    int fuel = 0;
    for (int &mass : masses) {
        int module_fuel = mass / 3 - 2;
        fuel += module_fuel;
        mass = module_fuel;
    }

    print_result("Part one", "Naive fuel requirement", fuel, 3302760);

    bool mass_remaining = true;
    while (mass_remaining) {
        mass_remaining = false;

        for (int &mass : masses) {
            int fuel_fuel = mass / 3 - 2;
            if (fuel_fuel > 0) {
                mass_remaining = true;
                fuel += fuel_fuel;
                mass = fuel_fuel;
            }
        }
    }

    print_result("Part two", "Real fuel requirement", fuel, 4951265);
}
